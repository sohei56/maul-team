...
// .app root and the build machine's absolute .build path, then fatalErrors —
// a distributed CLI-built .app would crash on first symbol access. Probe
// Contents/Resources (where make-app.sh copies SwiftPM resource bundles)
// first; Bundle.module still serves `swift build` / `swift test` runs.
...
public extension Image {
    ...
    init(symbol: String) {
        self.init(symbol, bundle: symbolsBundle)
    }
...
public extension NSImage {
    ...
    static func symbol(named: String) -> NSImage? {
        symbolsBundle.image(forResource: named)
    }
