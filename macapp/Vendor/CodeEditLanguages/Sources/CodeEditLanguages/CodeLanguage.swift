...
public struct CodeLanguage {
    ...
    // MAUL PATCH: the SwiftPM-generated Bundle.module accessor only probes the
    // .app root and the build machine's absolute .build path, so a distributed
    // CLI-built .app can never locate the resource bundle (and would crash via
    // the accessor's fatalError). Probe Contents/Resources — where make-app.sh
    // copies SwiftPM resource bundles — before falling back to Bundle.module
    // (which still serves `swift build` / `swift test` runs).
    ...
    internal func queryURL(for highlights: String = "highlights") -> URL? {
        // MAUL PATCH: upstream unconditionally prefixes "Resources/", which is
        // only correct for Xcode-built bundles (resource dir nested one level
        // deeper). SPM CLI-built bundles already end their resourceURL at
        // .../Resources, doubling the segment and silently killing every
        // highlight query. Prefer the layout that actually exists on disk.
        guard let base = resourceURL else { return nil }
        let relative = "tree-sitter-\(tsName)/\(highlights).scm"
        let nested = base.appendingPathComponent("Resources/\(relative)")
