...
`make-app.sh` builds the binary, assembles `build/ScrumTeam.app` with an
...
`swift build`) produce the bare Mach-O at `.build/<config>/ScrumTeam` with no
...
The `build/ScrumTeam.app` checked into your working tree may be **stale** — it
