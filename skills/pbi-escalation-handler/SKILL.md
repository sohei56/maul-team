...
| `merge_detector_regression` | Read `.scrum/pbi/<pbi-id>/detector-regression.log`; every line is prefixed with the guarded class identity. **Violation lines** → the PBI reintroduced that defect class three times; treat as the `merge_regression` row (split / redesign / hold), scoped to the class. **`DETECTOR COULD NOT EXECUTE`** → the ratchet is broken, not the PBI; that is a framework/tooling defect, so escalate to the PO seat with the two options: repair the detector (a new detector PBI) or un-guard the class via `.scrum/scripts/update-audit-ledger.sh set-status --identity <identity> --status open`, which returns it to LLM audit scope and is recorded in the ledger. Never un-guard merely to unblock the merge queue. |
| `teammate_unrecoverable` | The owning Developer teammate died and the SM's third recovery attempt failed (`../../agents/scrum-master.md` § health check step (e)) — the pipeline itself never reported a verdict, so no findings exist to triage. Confirm the teammate is really gone (`TaskGet` for this session; cross-session, a `pipeline.log` tail whose mtime is not advancing plus no newer stage review). Then inspect the PBI's last completed stage from `.scrum/pbi/<pbi-id>/state.json` and run the standard Step-4 full retry with a fresh Developer (round counters reset, status → `in_progress_design`) — the work is resumable because every stage artifact is on disk. Retry **once**; a second `teammate_unrecoverable` on the same PBI means the failure is environmental, not incidental → human-escalate with the two spawn failures as evidence. |
...
| `merge_detector_regression`, **`DETECTOR COULD NOT EXECUTE` branch only** (a violation branch uses the `merge_regression` shape) | `kind=escalation_choice options=[repair_detector,unguard_class,park] recommendation=<...>` — `repair_detector` files a new detector PBI for the named class and parks this one; `unguard_class` runs `set-status --identity <identity> --status open`; `park` writes to `.scrum/po/attention.md` and flips the PBI to `blocked`. |
