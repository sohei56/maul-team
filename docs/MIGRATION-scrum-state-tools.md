...
Agents must no longer edit `.scrum/*.json` directly. All writes flow through validated wrapper scripts under `.scrum/scripts/`. Mutating wrappers (`atomic_write`) take a directory lock, apply a `jq` expression, validate the result against a JSON Schema in `docs/contracts/scrum-state/`, and write atomically (`tmp` + `mv`). The `init-*` seeders (`atomic_create`) validate and write atomically the same way but take **no** directory lock — initial file creation is not a concurrent mutation (`scripts/scrum/lib/atomic.sh`). A `PreToolUse` hook blocks bypass attempts (`Write`, `Edit`, raw redirects, `jq -i`, `sed -i`) on `.scrum/**/*.json`.
...
| `jq -n '{items:[],next_pbi_id:1,product_goal:"..."}' > .scrum/backlog.json` (Requirement Definition step 6 seed) | `.scrum/scripts/init-backlog.sh [--product-goal <text>]` (idempotent; no-op if `.scrum/backlog.json` already exists) |
...
**Exception:** `merge-pbi.sh` publishes its own `0/1/2/3` contract instead (its subshell guards exist precisely to convert `64`/`66` into `1`), because the SM branches its recovery on those codes — canonical in [`skills/pbi-merge/SKILL.md`](../skills/pbi-merge/SKILL.md) § Steps.
...
Read access is **not** enforced. `cat .scrum/state.json | jq ...` is fine. The schemas under `docs/contracts/scrum-state/` are the read-side contract — clients (the dashboard, hooks, sub-agents) should validate or assume the documented shape. One read has a sanctioned reader anyway — per-PBI liveness goes through `.scrum/scripts/pbi-idle.sh` (§ Mapping), because the `0 = never observed` sentinel underneath it is easy to misread as "idle since 1970" or to paper over with a `now` fallback that reports a dead PBI as freshly active.
...
6. **`state.json.current_sprint_id` is a mirror.** It duplicates the presence/id of `.scrum/sprint.json`: `init-sprint.sh` writes both, and `rollover-sprint.sh` removes `sprint.json` while the mirror is cleared separately. The mirror is a convenience for readers that only need the current Sprint id without opening `sprint.json`; the long-term direction is to derive `current_sprint_id` from `sprint.json` presence/id and drop the stored field. Removal is deferred pending a compatibility migration for readers that consult `state.current_sprint_id` today.
...
The doc-only PBI flow (`skills/pbi-pipeline/SKILL.md` § Stages,
`docs/data-model.md` § kind=docs override)
...
## v6 → v7: the codebase-audit class ledger (2026-09-06)

A new SSOT file, `.scrum/audit-ledger.json`
(`audit-ledger.schema.json`), with a new sole writer,
`update-audit-ledger.sh`. It holds one entry per defect **class** the
codebase audit has ever seen, keyed by the same `audit_identity`
`backlog.json` already carries.

This is **additive**: no existing schema, wrapper, or field changes, and
a project that never writes the file behaves exactly as before. The
reason it is a state file rather than a report is that the two things
the audit most needed to remember — the class identity and its
occurrence set — used to live only where they could not survive: the
identity on whichever PBI happened to be open (so a class with no open
PBI had no key to reuse), and the occurrences in `--description` prose
(so "did the sweep shrink?" was unanswerable, and re-transcription lost
most of the list every round).

Two shapes deviate from the obvious minimum, both deliberately:
`pbi_ids[]` holds `{id, role}` objects rather than plain ids, because
promotion to `guarded` has to know *which* merged PBI wired the
detector; and `detector` is an object carrying its own
`verified_at` / `verified_exit` stamp rather than a boolean, because the
wrapper must not trust a flag an agent typed — it re-runs
`run-detectors.sh --check` and writes the result it observed.

`guarded` is the one status with teeth, and its preconditions are
