...
Each schema corresponds to one file under `.scrum/` and is the single source of truth for its on-disk shape today. **Normative target:** the validated wrapper scripts (`.scrum/scripts/*.sh` in deployed projects; `scripts/scrum/*.sh` in this framework's source tree) validate against these schemas on every write. Three documented exceptions to that rule (see the table below): (1) the hot-path, hook-owned appenders — `communications.json`, `dashboard.json`, `autonomy.json` (rows for those files) — validate the schema only when the file is first created (or on rotation), not per append, to keep the latency-sensitive path cheap; (2) `stop-gate.json`, `attention.json`, and `attention-context.json` — their writers (`hooks/lib/stop-gate-state.sh`, `hooks/notification-attention.sh`) never validate against the schema per write, not even on create; they are only WARN-validated at launch by `migrate-state.sh`; and (3) readers (dashboard, hooks, statusline) do **not** re-validate — they trust the wrapper-written shape. Treat "wrappers validate on write" as the invariant to preserve; the per-append and reader skips are deliberate, table-documented carve-outs, not a license to add unvalidated writers.

| File                                | Schema                                | Permitted writers (`.scrum/scripts/*.sh`)             |
|-------------------------------------|---------------------------------------|-------------------------------------------------------|
| `.scrum/state.json`                 | `state.schema.json`                   | `init-state.sh` (initial seed), `update-state-phase.sh`, `init-sprint.sh` (sets `current_sprint_id`), `rollover-sprint.sh` (nulls `current_sprint_id` at Sprint rollover), `migrations/001-legacy-to-ssot.sh` |
| `.scrum/sprint.json`                | `sprint.schema.json`                  | `init-sprint.sh` (initial seed), `update-sprint-status.sh`, `set-sprint-developer.sh`, `freeze-sprint-base.sh`, `migrations/001-legacy-to-ssot.sh` |
| `.scrum/backlog.json`               | `backlog.schema.json`                 | `init-backlog.sh` (initial seed), `update-backlog-status.sh` (validates the status against the schema's enum at runtime — no hardcoded copy — and additionally gates the transition into `refined` on a non-empty `demo_plan` for `kind=code` items), `set-backlog-item-field.sh`, `add-backlog-item.sh`, `migrations/001-legacy-to-ssot.sh`, `migrations/002-add-kind-field.sh` (idempotent atomic `kind: "code"` backfill), `migrations/005-add-audit-identity.sh` (lifts the legacy `audit-id:` marker out of `description` into the `audit_identity` field), `migrations/006-add-audit-severity.sh` (back-fills `audit_severity` from the title's `:<Severity>]` suffix, `Medium→high`); all migrations auto-run by `migrate-state.sh` at launch; also `mark-pbi-ready-to-merge.sh`, `mark-pbi-merged.sh`, `mark-pbi-merge-failure.sh`, `begin-impl-round.sh`, `resolve-integrity-fail.sh` (which delegate to `update-backlog-status.sh`; the resolver is the permitted higher-level Integrity-FAIL caller) |
...
| `.scrum/pbi/<id>/state.json`        | `pbi-state.schema.json`               | `init-pbi-state.sh` (initial), `update-pbi-state.sh` (low-level), `begin-impl-round.sh` (round bump + status transition); higher-level callers: `create-pbi-worktree.sh`, `commit-pbi.sh`, `mark-pbi-ready-to-merge.sh`, `mark-pbi-merged.sh`, `mark-pbi-merge-failure.sh`, `resolve-integrity-fail.sh` (validates Integrity aggregates and owns FAIL → next-Round/escalated transitions) |
| `.scrum/config.json`                | `config.schema.json`                  | The `merge_regression` block is written via `set-merge-regression-command.sh` (the sanctioned wrapper — sets `command` / `accepted_none`, invoked at Sprint Planning; also named in `config.schema.json`). All remaining keys: user (manual edit, interactive mode) **or** `scrum-start.sh --autonomous` (merges `po_mode`, `po`, `autonomous` defaults). Agent direct edits are blocked by `pre-tool-use-scrum-state-guard.sh`. |
...
| `.scrum/po/decisions.json`          | `po-decisions.schema.json`            | `append-po-decision.sh` only. Append-only; ids auto-assigned (`dec-NNNN`). Wrapper rejects blank evidence and requires evidence for `kind ∈ {demo_acceptance, sprint_acceptance, uat_item, release_decision}`; `sprint_acceptance` additionally requires `--sprint` and exact `approve\|reject`. It also enforces the green-tests gate for `release_decision=go`, and guard (d): a `defect_triage` whose `decision` is exactly `reject` must carry both `--audit-identity` and `--audit-severity` (an unmatchable suppression silently loses its own effect). |
...
| `.scrum/attention.json`             | `attention.schema.json`               | `hooks/notification-attention.sh` only (Notification sets `pending: true`; UserPromptSubmit clears it). **No `.scrum/scripts/*.sh` wrapper** — hook process, outside the guard's intercept, on the notification hot-path. Atomic tmp + mv; never validated per write, only WARN-validated at launch by `migrate-state.sh`. Read by the Mac app, which treats missing / unparseable / `pending != true` alike as "nothing is waiting". |
| `.scrum/attention-context.json`     | `attention-context.schema.json`       | `hooks/notification-attention.sh` only, on Stop (reached via `hooks/stop-dispatch.sh`, best-effort). Single-record message-recovery cache for the `attention.json` banner text; overwritten by every Stop from any session. Same no-wrapper / atomic / WARN-only-at-launch treatment as `attention.json`. |

Orchestrators (`merge-pbi.sh`, `merge-main-into-pbi.sh`, `safe-switch-to-main.sh`, `cleanup-pbi-worktree.sh`, `rollover-sprint.sh`, `migrate-state.sh` and its `migrations/*.sh`) drive git operations and the writers above; they do not bypass the schema-validated writes. `rollover-sprint.sh` additionally nulls `state.json.current_sprint_id` (a schema-validated `state.json` write — see its row above) and then deletes `.scrum/sprint.json` after archiving it (the sole sanctioned state-file deletion; refuses unless `sprint.json.status == "complete"`). `commit-integration-tests.sh` (used by the `integration-tests` skill) is a git-only commit wrapper for the target project's test assets — it writes no `.scrum/*.json` file, so it has no row above; see `MIGRATION-scrum-state-tools.md` § Worktree / merge governance wrappers. `pbi-idle.sh` has no row for the same reason in the other direction: it is a read-only reporter over `backlog.json` and the per-PBI activity signals in `scripts/scrum/lib/activity.sh`, and writes nothing at all (see `MIGRATION-scrum-state-tools.md` § Mapping).
...
- **No-schema SSOT siblings.** Several runtime files live under
  ...
  tool surface (launcher script or hook process),
