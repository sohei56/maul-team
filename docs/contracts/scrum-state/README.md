...
| `.scrum/po/decisions.json`          | `po-decisions.schema.json`            | `append-po-decision.sh` only. Append-only; ids auto-assigned (`dec-NNNN`). Wrapper enforces evidence requirement for `kind ∈ {demo_acceptance, uat_item, release_decision}`, the green-tests gate for `release_decision=go`, and guard (d): a `defect_triage` whose `decision` is exactly `reject` must carry both `--audit-identity` and `--audit-severity` (an unmatchable suppression silently loses its own effect). |
