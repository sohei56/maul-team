...
|-------------------------------------|---------------------------------------|-------------------------------------------------------|
| `.scrum/state.json`                 | `state.schema.json`                   | `init-state.sh` (initial seed), `update-state-phase.sh`, `init-sprint.sh` (sets `current_sprint_id`), `migrations/001-legacy-to-ssot.sh` |
...
- **No-schema SSOT siblings.** Several runtime files live under
  ...
  target), and `.scrum/session-map.json` (autonomous-mode iteration →
  session id ledger). They match the guard's `.scrum/**/*.json` pattern but
