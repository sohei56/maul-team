...
The axes are **not named catalog agents**: each is a **general-purpose
...
into `.scrum/reviews/static-analysis-r{n}.json`, which the **`redundancy`
audit axis** cites as ground truth (it is the passes' sole Sprint-level
consumer). Per-language tool selection:
`skills/cross-review/SKILL.md` § Steps, step 5.
...
The five aspect reviewers below run **per-PBI, inside the pipeline**, as
the Integrity stage — the final quality gate before ready-to-merge
