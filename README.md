...
6. **Cross-Review** — once all PBIs are merged, the SM closes every Sprint's PBIs; every third Sprint (`N % 3 == 0`) it also runs a non-blocking whole-repo 4-axis `codebase-audit` (spec-conformance, logic-defect, redundancy, product-security). Every finding is graded by severity and put to you; the ones you accept become draft PBIs for the next Sprint
