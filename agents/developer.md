...
- **No branch ops.** Never run `git checkout -b`, `git switch -c`, `git branch <name>`, `git push`, `git merge`, `git rebase`, or `git worktree add -b` directly. Use `.scrum/scripts/*` wrappers (`commit-pbi.sh` for commits, `mark-pbi-ready-to-merge.sh` for handoff). This list is team policy and is wider than the machine-enforced subset — `../hooks/pre-tool-use-no-branch-ops.sh` is the SSOT for what is actually blocked.
